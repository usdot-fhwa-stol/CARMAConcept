...
public class ObuDsrcProcessor implements Runnable {
    ...
    /**
     * For use if the default AppConfig object is going to be used
     */
    ...
    /**
     * Allows specification of a custom (derived) config object
     * @param config
     */
    public ObuDsrcProcessor(IAppConfig config){
        setupProperties(config);
    }
    ...
    private void setupProperties(IAppConfig config) {
        ...
        this.obuUdpInPort = config.getIntValue("asd.inport");
        this.mabUdpOutPort = config.getIntValue("mab.dsrc.outport");
