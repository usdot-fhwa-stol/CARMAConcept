package gov.dot.fhwa.saxton.carmasecondary.consumers.gps;
...
public class GpsScheduledMessageConsumer implements Runnable {
    ...
    private IAppConfig config = null;
    ...
    private NioUtils nioUtils = new NioUtils(socketChannel);
    private SpeedCmdProcessor speedCmdProcessor;
    ...
    public GpsScheduledMessageConsumer(IAppConfig ac) {
        config = ac;
        setupProperties(config);
    }

    private void setupProperties(IAppConfig config) {
        this.serverIp = config.getProperty("gps.host");
        this.serverTcpPort = config.getIntValue("gps.port");
        this.clientUdpPort = config.getIntValue("gps.udpport");
        this.mabUdpPort = config.getIntValue("mab.gps.port");
            ...
            while (!bShutdown.get())  {
                ...
                if (bytesRead > 0)   {
                    ...
                    if (gpsPacket[0] != 0x81 && gpsPacket[1] != 0x03)   {
                        ...
                        //the GlobalPose MethodID = 7
                        if (gpsPacket[0] == 0x21 && gpsPacket[1] == 0x07){
                        	
                        	//get current vehicle(lead vehicle)'s gps coordinate
                            double myLat = nioUtils.decodeLatLong(gpsPacket, 11);
                            double myLon = nioUtils.decodeLatLong(gpsPacket, 15);	
                            speedCmdProcessor.generateSpeedCmd(myLat, myLon);
                        }                           
                    }
                }
    ...
    public void sendScheduleMessages() throws Exception   {
        ...
        int iFrequency = config.getIntValue("gps.frequency");
     ...
     */
    public boolean initialize(SpeedCmdProcessor proc)  {

        Boolean bResult = false;

        speedCmdProcessor = proc;
