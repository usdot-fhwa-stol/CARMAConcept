...
public class LoggerManager {
    private static HashMap<Class, Logger> loggerInstances = new HashMap<Class, Logger>();
    private static String outputFilename = "log.txt";
    private static ArrayList<LogListener> globalListeners = new ArrayList<LogListener>();
    ...
    private static IComponentVersion versionInfo = null;

    /**
     * Allows specification of version ID info for the calling component
     * @param ver specifies pertinent version ID information
     */
    public static void setVersionId(IComponentVersion ver) {
        versionInfo = ver;
    }
    ...
    public synchronized static void writeToDisk() throws IOException {
        ...
        if (firstWrite) {
            ...
            String version = "*** UNKNOWN SOFTWARE IDENTITY ***";
            if (versionInfo != null) {
                version = versionInfo.toStringWithDependencies();
            }
            out.println(version);
            out.println(fmt.print(dt));
            out.println();
